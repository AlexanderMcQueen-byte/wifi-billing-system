// Full client app adapted from main public/app.js
const packages = {
...
function normalizePhone(phone) {
  ...
  if (/^\+254\d{9}$/.test(value)) { return value.replace('+', ''); }
  return null;
}

function renderPackages() {
  const el = document.getElementById('packagesWrap');
  if (!el) return;
  el.innerHTML = Object.entries(packages)
    .map(([key, value]) => `
      <label class="package">
  ...
  el.addEventListener('change', (event) => {
    if (event.target.name === 'package') selectedPackage = event.target.value;
  });
}
  ...
  poller = setInterval(async () => {
    try {
      ...
      if (payload.data.internetReady) {
        ...
        setTimeout(() => { window.location.href = 'http://logout.net'; }, 2500);
...
if (paymentForm) {
  paymentForm.addEventListener('submit', async (event) => {
    ...
    if (!macAddress) { setStatus('MAC address missing from hotspot redirect URL.', 'error'); return; }
    payBtn.disabled = true;
    setStatus('Sending STK push. Enter your M-Pesa PIN on your phone...', 'warn');
    try {
      const response = await fetch('/api/pay', {
        method: 'POST', headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ phone, macAddress, ipAddress, packageKey: selectedPackage })
      });
      const payload = await response.json();
      if (!response.ok || !payload.success) throw new Error(payload.message || 'Failed to initiate payment');
      setStatus('STK push sent. Please complete payment on your phone.', 'warn');
      await pollStatus(phone);
    } catch (error) {
      payBtn.disabled = false; setStatus(error.message, 'error');
    }
  });
}

renderPackages();
if (clientMeta) clientMeta.textContent = `Router session detected | MAC: ${macAddress || 'N/A'} | IP: ${ipAddress || 'N/A'}`;
// Lightweight shim that delegates to the main app.js served by the primary `public/` directory.
console.info('wifi-hotspot-portal public/app.js shim loaded');
